...
export default function ProfilDesa() {
      ...
      <main className="min-h-screen bg-[#f6f3ee]">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <p className="text-[#7a6e5a]">Memuat data...</p>
          </div>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-[#f6f3ee] text-[#3e3323]">
      <Helmet>
        <title>Profil Padukuhan | Portal Padukuhan Kayen</title>
        <meta
          name="description"
          content="Informasi profil Padukuhan Kayen mencakup sejarah padukuhan, struktur organisasi, statistik, dan gambaran wilayah."
        />
      </Helmet>
        ...
        <div className="relative mx-auto flex min-h-[520px] max-w-7xl items-end px-6 pb-16 pt-14 md:px-8">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 text-base leading-8 text-white/80 md:text-lg">
              Wujud transparansi dan modernisasi pelayanan warga yang tetap berakar pada gotong royong,
              budaya, dan kekuatan komunitas padukuhan.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-14 px-6 py-20 md:px-8 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="space-y-6">
          ...
          <div className="space-y-4 leading-8 text-[#7a6e5a]">
            <p>
              Padukuhan Kayen didirikan pada awal abad ke-20 dari pemukiman kecil di kaki perbukitan
              yang dikelilingi lahan pertanian subur. Sejak awal, padukuhan ini tumbuh dengan semangat
              musyawarah dan kerja bersama antarkeluarga.
            </p>
            <p>
              Dalam perjalanannya, padukuhan berkembang dari kawasan agraris tradisional menjadi
              padukuhan yang mulai mengadopsi layanan digital, tanpa meninggalkan nilai adat,
              solidaritas sosial, dan kearifan lokal yang diwariskan para tetua.
            </p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {profileStats.map((stat) => (
              ...
              >
                <p className="font-heading text-3xl font-extrabold text-[#7a5b0a]">{stat.value}</p>
                <p className="mt-2 text-xs uppercase tracking-[0.18em] text-[#8e7d63]">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              <div className="flex flex-col items-center">
                <div className="relative rounded-[1.7rem] bg-[#7a5b0a] px-8 py-7 text-center text-white shadow-[0_28px_50px_-30px_rgba(122,91,10,0.95)]">
                  <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full border-4 border-white/20 bg-white/10 font-heading text-2xl font-bold">
                    {villageHead.name.slice(0, 2).toUpperCase()}
                  </div>
                  <h3 className="font-heading text-2xl font-bold">{villageHead.name}</h3>
                  <p className="mt-1 text-sm uppercase tracking-[0.16em] text-white/75">{villageHead.role}</p>
        ...
        <div className="order-1 space-y-7 lg:order-2">
          <div>
            ...
            <p className="mt-4 leading-8 text-[#7a6e5a]">
              Padukuhan Kayen berada di wilayah perbukitan subur dengan persawahan produktif, sumber
              air yang memadai, dan konektivitas antardusun yang terus dikembangkan.
            </p>
          </div>
          ...
          <div className="space-y-5">
            {villageBoundaries.map((item) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-heading text-xl font-bold text-[#5a430d]">{item.direction}</h3>
